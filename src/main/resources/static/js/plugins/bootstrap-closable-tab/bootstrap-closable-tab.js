var closableTab = {
    ...
    frameLoad: function (frame) {
        ...
        $(frame).height((document.documentElement.clientHeight)-mainheight);
    },
    //添加tab
    addTab: function (tabItem) { //tabItem = {id,name,url,closable}
        ...
        if (!$('#' + id)[0]) {
            ...
            if (tabItem.closable) {
                li_tab = li_tab + '<i class="glyphicon glyphicon-remove small" data-toggle="context" tabclose="' + id + '" style="position: absolute;right:4px;top: 4px;"  onclick="closableTab.closeTab(this)"></i></a></li> ';
            } else {
                li_tab = li_tab + '</a></li>';
            }
